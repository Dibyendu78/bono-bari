...
export function ButtonAnimatedText({
  children,
  className,
}: {
  children: React.ReactNode;
...
// ─── PLANS DATA ───────────────────────────────────────────────────────────
export type Feature = { text: string; textBn?: string; included: boolean };
export type Plan = {
  name: string;
  nameBn?: string;
  price: string;
...
export const bonoBariPlans: Plan[] = [
  {
...
export function PricingCard({
  ...
  const displayName = isBn ? (plan.nameBn || plan.name) : plan.name;
  const displayDesc = isBn ? (plan.descriptionBn || plan.description) : plan.description;
  ...
  const displayBtnText = isBn ? (plan.buttonTextBn || plan.buttonText) : plan.buttonText;
...
📝 *প্যাকেজ বিবরণ:* ${plan.descriptionBn || plan.description}
